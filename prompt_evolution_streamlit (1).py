...
# Import your existing APIManager
...
class PromptEvolutionAPIAdapter:
    """Adapts your APIManager to work with the evolution engine"""
    
    def __init__(self, api_manager, group_name: str, 
                 asset_name: str = 'Managed Service (GPT)', 
                 deployment_name: str = '4o_default'):
    ...
    def evaluate(self, prompt: str) -> str:
        """Sync wrapper for evaluation calls"""
        return asyncio.run(self.api_manager.chat(
    ...
    def optimize(self, prompt: str) -> str:
        """Sync wrapper for optimization calls"""
        return asyncio.run(self.api_manager.chat(
            group_name=self.group_name,
            text_to_send=prompt,
            asset_name=self.asset_name,
...
# Sidebar - API Connection & Configuration
with st.sidebar:
    ...
    with st.form("api_config"):
        ...
        # Username input
        user_id = st.text_input("User ID:", value="7756832", help="Enter your user ID (e.g., A123456)")
        full_username = f"NAEAST\\{user_id}" if user_id else "NAEAST\\"
        
        # Password input
        password_input = st.text_input("Password:", type="password", help="Enter your network password")
        
        # Token input (optional)
        token_input = st.text_input("Token (Optional):", type="password", 
                                    help="Paste your API token here, or leave blank to authenticate with password")
        
        if st.form_submit_button("Connect to API"):
            if user_id.strip() and (token_input.strip() or password_input.strip()):
                try:
                    # Create token file path
                    current_dir = os.path.dirname(os.path.abspath(__file__))
                    token_dir = os.path.join(current_dir, "tokens")
                    os.makedirs(token_dir, exist_ok=True)
                    token_file_path = os.path.join(token_dir, f"token_{user_id}.txt")
                    ...
                    st.success("✅ API Manager Connected!")
                    st.rerun()
                ...
                st.error("⚠️ Please enter User ID and either Password or Token")
    
    # Show connection status
    if st.session_state.api_manager is not None:
    ...
    group_name = st.text_input("Group Name", value="prompt_evolution", 
                               help="API group name for chat calls")
    deployment_name = st.selectbox("Deployment", ["4o_default", "gpt-4", "gpt-3.5-turbo"])
...
with tab1:
    ...
    if st.button("▶️ Start Evolution", type="primary", disabled=not initial_prompt):
        ...
        if st.session_state.api_manager is None:
            st.error("⚠️ Please connect to API in the sidebar first!")
            st.stop()
        
        st.session_state.evolution_running = True
        
        try:
            # Create adapter using your existing API manager
            api_adapter = PromptEvolutionAPIAdapter(
